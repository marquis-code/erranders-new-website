<!-- <template>
    <main>
        <div class="flex justify-between items-start">
            <div>
                <svg @click="router.back(-1)" class="cursor-pointer" width="68" height="24" viewBox="0 0 68 24"
            ...
            <div>
                <p class="text-[#033A83] font-medium text-sm py-0 my-0">STEP {{ activeStep === 'personal-info' ? `02` :
                    `03` }}/03
                </p>
                <h1 class="font-semibold text-sm text-gray-900 py-0 my-0">
                    {{ activeStep === 'personal-info' ? `Personal Info` : `Residency Info` }}
                </h1>
            </div>
        ...
        <div>
            <h1 class="text-3xl lg:text-4xl font-semibold max-w-sm text-gray-800">{{ activeStep === 'personal-info' ?
                    `Register
                Errander
                account` : `Complete your Errander
                profile`}}</h1>
            <p class="font-light leading-loose max-w-lg">For the purpose of industry regulation, your details are
                required.</p>
        </div>
        <div v-if="activeStep === 'personal-info'" class="space-y-6 pt-10">
            <div>
                <label for="email" class="block text-sm font-medium leading-6 text-[#0D0C22]">Name</label>
                <div class="mt-0.5">
        ...
        <div v-if="activeStep === 'residency-info'" class="space-y-6 pt-10">
            <div>
                <label for="email" class="block text-sm font-medium leading-6 text-[#0D0C22]">Phone
                    number</label>
                <div class="mt-0.5">
                    <input type="email" name="email" id="email"
                        class="block w-full rounded-md border border-gray-100 p-3 py-3.5 text-gray-900 shadow-sm   placeholder:text-gray-400 placeholder:font-light outline-none "
            ...
            <div>
                <label for="email" class="block text-sm font-medium leading-6 text-[#0D0C22]">Home address</label>
                <div class="mt-0.5">
                    <input type="email" name="email" id="email"
                        class="block w-full rounded-md border border-gray-100 p-3 py-3.5 text-gray-900 shadow-sm   placeholder:text-gray-400 placeholder:font-light outline-none "
                        placeholder="you@example.com">
            ...
            <div>
                <label for="email" class="block text-sm font-medium leading-6 text-[#0D0C22]">City of residence</label>
                <div class="mt-0.5">
                    <input type="email" name="email" id="email"
                        class="block w-full rounded-md border border-gray-100 p-3 py-3.5 text-gray-900 shadow-sm   placeholder:text-gray-400 placeholder:font-light outline-none "
                        placeholder="you@example.com">
            ...
            <div class="w-full pt-6">
                <button type="button" @click="createUser" :disabled="processing"
                    class="text-white disabled:cursor-not-allowed disabled:opacity-25 bg-[#0BCA63] px-6 py-3.5 clear-left w-full rounded-full">{{ processing
                    ? 'saving...' : 'Next'}}</button>
            </div>
        </div>
    </main>
...
<template>
    <form @submit.prevent="handleRegister">
        ...
        <div v-if="activeStep === 'personal-info'" class="space-y-6 pt-6">
            <div>
                <label for="name" class="block text-sm font-medium leading-6 text-[#0D0C22]">Name</label>
                <div class="mt-0.5">
                    <input type="text" name="name" id="name" v-model="registerPayload.name"
                        class="block w-full rounded-md border border-gray-100 p-3 py-3.5 text-gray-900 shadow-sm   placeholder:text-gray-400 placeholder:font-light outline-none "
                        placeholder="Enter your name">
                </div>
            </div>
            <div>
                <label for="email" class="block text-sm font-medium leading-6 text-[#0D0C22]">Email address</label>
        ...
        <div v-if="activeStep === 'residency-info'" class="space-y-6 pt-6">
            <div>
                <label for="phone" class="block text-sm font-medium leading-6 text-[#0D0C22]">Phone number
                    number</label>
                <div class="mt-0.5">
            ...
            <div>
                <label for="businessAddress" class="block text-sm font-medium leading-6 text-[#0D0C22]">Home address</label>
                <div class="mt-0.5">
                    <input type="text" name="businessAddress" id="businessAddress" v-model="registerPayload.homeAddress"
                        class="block w-full rounded-md border border-gray-100 p-3 py-3.5 text-gray-900 shadow-sm   placeholder:text-gray-400 placeholder:font-light outline-none "
...
<script lang="ts" setup>
import { useErranderSignup } from '@/composables/onboarding/erranderSignup'
const { registerPayload, handleRegister, loading, isFirstFormEmpty, isSecondFormEmpty } = useErranderSignup()
import eyeOpen from '@/assets/icons/eye-open.svg'
import eyeClose from '@/assets/icons/eye-close.svg'
const router = useRouter()
const showPassword = ref(false)
